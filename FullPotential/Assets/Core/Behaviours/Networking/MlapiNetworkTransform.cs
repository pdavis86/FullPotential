...
{
    public class MlapiNetworkTransform : NetworkBehaviour
    {
        ...
        private void Update()
        {
            if (IsOwner)
            {
                if (NetworkManager.Singleton.ServerTime.TimeAsFloat - m_LastSendTime >= (1f / FixedSendsPerSecond) && (Vector3.Distance(transform.position, m_LastSentPos) > MinMeters || Quaternion.Angle(transform.rotation, m_LastSentRot) > MinDegrees))
                {
                    m_LastSendTime = NetworkManager.Singleton.ServerTime.TimeAsFloat;
                    m_LastSentPos = transform.position;
                    m_LastSentRot = transform.rotation;
        ...
        private void SubmitTransformServerRpc(Vector3 position, Vector3 eulerAngles, ServerRpcParams rpcParams = default)
        {
            ...
            if (EnableRange)
            {
                for (int i = 0; i < NetworkManager.Singleton.ConnectedClientsList.Count; i++)
                {
                    ...
                    if ((receiverPosition == null || senderPosition == null && NetworkManager.Singleton.ServerTime.TimeAsFloat - info.LastSent >= (1f / FixedSendsPerSecond)) || NetworkManager.Singleton.ServerTime.TimeAsFloat - info.LastSent >= GetTimeForLerp(receiverPosition.Value, senderPosition.Value))
                    {
                        info.LastSent = NetworkManager.Singleton.ServerTime.TimeAsFloat;
